...
public class ThreeMusketeers {
    ...
    protected void move(final Agent agent) { // TODO
        Move move = new Move(agent.getMove()); // create a new move using the copy constructor of move, passing agent.getMove and declare it move
        moves.add(move); // add the move to the moves list by calling moves.add(move)
        board.move(move); // moves the piece object on the board by calling board.move(move)
    }
    ...
    private void undoMove() { // TODO
        Move removedMove = this.moves.get(this.moves.size()-1); // get the recent move that was done, and if you wish to undo, but getting access to the last element in the moves list, and assign it to removedMove
        moves.remove(removedMove); // remove the move from the moves list, by calling moves.remove(removedMove)
        board.undoMove(removedMove); // undo the move on the board, by calling board.undoMove(removedMove)
    }
